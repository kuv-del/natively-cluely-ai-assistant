...
1. 🟢 **1.22** Weekly calendar view in main UI (replaces activity feed top widgets) — **SHIPPED 2026-04-22 (overnight)**
...
| 1.22 | ~~**Weekly calendar view in main UI**~~ — **SHIPPED 2026-04-22 (overnight build)**. Replaced the top hero grid (next-meeting card + calendar-connected card) in `Launcher.tsx` with `<WeekView>` (`src/components/WeekView.tsx`). 7-column Sun–Sat grid, time axis 7am–9pm @ 48px/hr, events absolutely positioned with height = duration, left border + faded background = `colorHex`, RSVP icon prefix (☑/□/⚠️). Header has prev/this-week/next nav + "Clean \| Everything" toggle. Click event → existing `handleOpenUpcomingMeeting()` (→ DealDetails via `convex-get-meeting-profile`). Backend: new `weekview:get-events` IPC reuses `CalendarManager` with refactored `fetchEventsInternal({ startTime, endTime, filterBlocked })` + new public `getEventsInRange()`. Clean mode applies the same `BLOCKED_PHRASES` blocklist as menu bar; Everything mode bypasses it (shows OOO, blocks, family events unfiltered). Hero header (My Natively title, Detectable toggle, Start Natively pill) preserved. **Refinements 2026-04-22:** (1) word-wrap event titles (normal+break-word); (2) transparent outer background; (3) dynamic time range (compute min/max from events, exclude AM/PM out blocks); (4) no-flash grid scaffold (always render, empty-state overlay); (5) full-width WeekView in Launcher (break out of max-w-4xl). | ✅ Shipped & refined 2026-04-22 |
