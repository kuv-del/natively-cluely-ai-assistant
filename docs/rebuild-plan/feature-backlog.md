...
| 1.17 | **Multi-party transcript swap** — when a calendar event has 3+ attendees, automatically replace the Natively transcript with Zoom's speaker-labeled transcript after the call. Natively's 2-channel "User/Interviewer" split works for 1:1 calls but can't name individual speakers in group calls. Zoom's transcript (already synced to Convex via `zoom-transcript-sync`) has per-speaker labels. Logic: post-meeting, check `event.attendees.length >= 3`. If true, wait for Zoom transcript to land in Convex (poll or webhook), then overwrite the Natively transcript row with Zoom's version. The Natively summary/coaching data generated during the live call is preserved — only the raw transcript is swapped. | Phase 1 |
