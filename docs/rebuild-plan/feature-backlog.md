...
**Priority order (remaining active items, 2026-04-23):**
1. 🟢 **1.22** Weekly calendar view in main UI — **SHIPPED + REFINED 2026-04-22/23**
2. 🔵 **1.25** GoBot-powered "Ask anything" search bar — **DESIGNED 2026-04-23, ready to build** (replaces Gemini RAG for AI queries; keeps local session search)
3. 🔴 **1.21** RSVP + calendar color on meeting feed — active priority
4. 🔴 **1.1** Storage architecture (Convex as source of truth — foundation for everything)
5. 🟠 **1.3** Knowledge base revamps (sales-reframing 13 premium modules)
6. 🟠 **1.6** Next Steps button + commands → HubSpot updates
7. 🟠 **1.7** Test HubSpot updates (wire approved Next Steps → HubSpot PATCH)
8. 🟠 **1.14** Deal grading assessment (Grade tab)
...
| 1.22 | ~~**Weekly calendar view in main UI**~~ — **SHIPPED 2026-04-22 (overnight) + REFINED 2026-04-23**. Replaced the top hero grid with `<WeekView>` (`src/components/WeekView.tsx`). 7-column Sun–Sat grid, PST/EST/CST timezone columns, per-calendar routing (Scalable→DealDetails, Matria→MatriaDetails, Family→KateDetails), muted earth-tone GCAL color mapping (11 colorIds), Banana events use raw title, event cards show contact name + company + type pill (Demo/Disc/Fup/Appt/School/etc), AM/PM block boundary lines. Activity feed removed from Launcher, WeekView fills full vertical space. v3 design system: `useV3Tokens` hook, `#FDFDFA` palette, Playfair Display true italic fonts (6 TTF files), earth-tone pill colors. KateDetails + MatriaDetails detail pages added. **Refinements 2026-04-23:** (1) block boundary lines — weekdays only (Mon–Fri), only am_out end + pm_out start, no weekend lines; (2) "morning meeting block" reclassified to other_block (was drawing phantom 10am line); (3) day headers `Mon \| 19` on one line; (4) today pill wraps full `Thu \| 23` label not just the number; (5) header cells use true flex centering (not padding/text-align); (6) `hourCount = endHour - startHour` (removed +1, no extra row past last event); (7) ResizeObserver watches `timeGridRef` directly (no manual offset math, eliminates blank row at bottom); (8) Launcher wrapper backgrounds switched from hardcoded `#FDFDFA` to `v3.bg` — dark mode now works correctly across the whole window. | ✅ Shipped 2026-04-22, refined 2026-04-23 |
...
| 1.25 | **GoBot-powered "Ask anything" search bar** — **DESIGNED 2026-04-23, ready to build.** Replaces the Gemini/local-RAG AI path in `GlobalChatOverlay` with a GoBot streaming endpoint. Two-path architecture: (a) **local path unchanged** — literal session search + meeting RAG (`ragQueryGlobal`) stays on Gemini as-is; (b) **AI Query (✨) path** — new `/ask` SSE endpoint on GoBot, auth `Bearer GATEWAY_SECRET`, body `{ query, meetingContext?, conversationHistory? }`. Natively pre-fetches meeting RAG chunks and passes as `meetingContext` before calling GoBot. GoBot uses full tool suite (Gmail, Calendar, HubSpot, Notion, Slack, Zoom, Convex) + Claude model routing (Haiku/Sonnet/Opus via existing `model-router.ts`). SSE events: `{type:"status"}`, `{type:"tool",name,status}`, `{type:"text",chunk}`, `{type:"done",model}`. Natively side: new `ask:gobot` IPC in `ipcHandlers.ts`, reads `GOBOT_LOCAL_URL` + `GATEWAY_SECRET` from env. GlobalChatOverlay: multi-turn conversation history (resets on close), tool status line during processing, model badge on each response, graceful fallback if GoBot unreachable. | Design done 2026-04-23 |
...
### 2026-04-23 — WeekView refinements + dark mode + v3 design system (continued)

~~**Block boundary lines — weekdays only, correct kinds only**~~ — Lines now render only for `am_out` (at end time) and `pm_out` (at start time) on Mon–Fri columns. Weekend columns get no lines. `sat_out`/`sun_out` blocks no longer draw lines. — shipped 2026-04-23

~~**"Morning meeting block" → other_block**~~ — `detectBlockKind()` in `CalendarManager.ts` previously classified "morning meeting block" as `am_out`, drawing a phantom line at its end time (10am Mon–Thu, 10:30am Fri). Reclassified to `other_block` — event stays invisible but draws no line. — shipped 2026-04-23

~~**Day headers: "Mon | 19" on one line**~~ — Day name and date number on a single row with `|` separator. Today pill wraps the full `Thu | 23` label (not just the number). Header cells use true flex centering (`display:flex, alignItems:center, justifyContent:center`) with `minHeight:44` — no more padding-based approximation. `alignItems:stretch` on the header row keeps all cells the same height. — shipped 2026-04-23

~~**hourCount off-by-one fix**~~ — `hourCount = endHour - startHour + 1` was adding one extra hour row past the latest event's ceiling. Changed to `endHour - startHour` in both the render path and the ResizeObserver. No more "10pm row" showing when the latest event ends at 9pm. — shipped 2026-04-23

~~**ResizeObserver watches timeGridRef directly**~~ — Previously watched `gridContainerRef` and subtracted magic offsets (`-50 - stripHeightPx - 16`) to estimate available height — imprecise, left a blank row at the bottom before the all-day strip. Now watches `timeGridRef` (the actual time grid div), so `hourPx = floor(containerHeight / hourCount)` with no manual math. Blank row eliminated. — shipped 2026-04-23

~~**Dark mode: Launcher wrapper backgrounds**~~ — Three hardcoded `style={{ background: '#FDFDFA' }}` instances in `Launcher.tsx` (outer div, top section, WeekView wrapper) replaced with `v3.bg` from `useV3Tokens`. Dark mode now flips the entire window correctly, not just the calendar grid. — shipped 2026-04-23

~~**v3 design system — overnight build (2026-04-22/23)**~~ — Full v3 design token system: `useV3Tokens` hook (`#FDFDFA`/`#1B1B1B` light/dark palette, Nunito Sans + Playfair Display). 6 Playfair Display TTF files (400/600/700 × roman/italic) with true italic, `font-feature-settings: "liga" 1, "dlig" 1`. Muted GCAL earth-tone color mapping (11 colorIds). Per-calendar routing: Scalable→DealDetails, Matria→MatriaDetails, Family→KateDetails. New `KateDetails.tsx` (Overview/Notes/Files/History tabs) + `MatriaDetails.tsx` (Summary/Profile/Grade/Meetings/Prep tabs). `DealDetails.tsx` and `MatriaDetails.tsx` refactored to use `useV3Tokens`. Activity feed removed from `Launcher.tsx`, WeekView fills full vertical space with `pb-4` gutter. Start button replaced with Playfair italic `Start >` black pill. Detectable toggle removed. — shipped 2026-04-22/23
